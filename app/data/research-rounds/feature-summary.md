# Defra Forms Research Summary by Feature

## Form Creation and Details

### Evolution of Form Creation

#### Initial Testing (Round 1 - May 2024)

- Form Name and Title

  - Confusion between form 'name' and 'title'
  - Users expect flexibility in naming as it's often decided collaboratively
  - Recommendation: Explicitly state editability of form names and clarify difference between name and title

- Lead Organisation

  - Clear but requires ability to select multiple organisations
  - Users work across multiple areas (e.g., Defra core, APHA, DAERA)
  - Recommendation: Enable multi-organisation selection

- Team Details and Email

  - Often no shared team email; reliance on personal Defra emails
  - Recommendation: Provide guidance on email use and facilitate easier updates

- Form Overview
  - Clear but uncertainty about GOV.UK publishing location
  - Recommendation: Clarify publication process and location

#### Refinement (Round 2 - May/June 2024)

- Form Overview
  - Clear, consistent with Whitehall experience
  - No issues identified

#### Further Improvements (Round 7 - October 2024)

- Team Details and Email
  - 'Shared team email' hint text confusion regarding personal email usage
  - Recommendation: Clarify definition of 'personal email address'

### Key Improvements Over Time

1. Terminology: Clarified difference between form name and title
2. Functionality: Added support for multiple organisations
3. Guidance: Improved email usage instructions
4. Clarity: Enhanced form overview and publication process explanation

## Form Editor

### Evolution of Question Creation

#### Initial Testing (Round 2 - May/June 2024)

- Confusion about relationship between creating pages and adding questions
- 'Page title' was unclear to participants
- Recommendation: Consider automatically creating pages as users add questions
- Recommendation: Add more explanatory text or examples

#### Major Redesign (Round 11 - March 2025)

- Users expected to write question before setting up options
- Recommendation: Allow question text entry before radio options

#### Refinement (Round 12 - March 2025)

- Moving question text before options improved logic and usability
- No usability issues observed; setup was clear

### Evolution of List Items and Options

#### Initial Testing (Round 2 - May/June 2024)

- 'Add Component' unclear to participants
- Recommendation: Clarify interaction or provide additional user training

#### Major Redesign (Round 11 - March 2025)

- 'Add a new radio option' page redirecting caused disruption
- Recommendation: Keep users on same page when adding multiple options

#### Refinement (Round 12 - March 2025)

- Users appreciated entering all options on one page
- Maintain single-page radio option entry for better drafting visibility

### Evolution of Hint Text and Short Description

#### Refinement (Round 12 - March 2025)

- Participants confused about purpose and difference between hint text and short description
- Recommendation: Provide clearer guidance or examples to distinguish between fields

### Evolution of Error Messages

#### Initial Testing (Round 7 - October 2024)

- Labels and copy clear, but interaction confusing
- 'Enter' or 'Please enter' ambiguity
- Recommendation: Standardize error message input prompts

#### Refinement (Round 8 - November 2024)

- 'Prefix' wording (e.g. "Enter", "Select") was confusing
- Recommendation: Set expectations for prefixes, offer flexibility, and provide clear guidance

### Evolution of Question Frequency

#### Initial Testing (Round 6 - August/September 2024)

- Users found 'How many times...' initially confusing
- Recommendation: Provide clearer context or examples, such as 'Add another child'

### Evolution of Short Label

#### Initial Testing (Round 6 - August/September 2024)

- Term 'short label' unclear without reading hint text
- Recommendation: Clarify label or directly indicate error message purpose

### Evolution of Saving Changes

#### Initial Testing (Round 6 - August/September 2024)

- Users unclear if saving was necessary before adding another question
- Recommendation: Clearer guidance on the need to save progress

### Evolution of Editor Summary

#### Initial Testing (Round 6 - August/September 2024)

- Very clear, intuitive, and helpful for reviewing form structure
- Recommendation: Maintain current design

### Evolution of Adding Multiple Questions

#### Initial Testing (Round 6 - August/September 2024)

- Option 'More than 1 question' clearer after initial use
- Recommendation: Maintain existing design; initial guidance needed for first-time users

### Evolution of Guidance and Markdown

#### Initial Testing (Round 6 - August/September 2024)

- Markdown helpful but requires reminders
- Recommendation: Provide easy-access markdown help links or tooltips

#### Refinement (Round 7 - October 2024)

- Unclear if markdown usage had limitations
- Recommendation: Provide detailed guidance on markdown capabilities

### Evolution of Navigation After Adding a Question

#### Initial Testing (Round 6 - August/September 2024)

- Unclear next steps after adding the first question
- Recommendation: Provide clear continuation prompts at bottom of page

### Evolution of Confirmation Page

#### Initial Testing (Round 6 - August/September 2024)

- Page visually crowded and overwhelming
- Recommendation: Revisit page layout for visual clarity and navigation consistency

### Evolution of Repeater Feature

#### Initial Testing (Round 6 - August/September 2024)

- Initial confusion but understood after interaction
- Recommendation: Research further to validate naming of this feature

#### Refinement (Round 7 - October 2024)

- Label "Repeater" clear after interaction but initially confusing
- Recommendation: Conduct further research to confirm appropriate naming

### Evolution of Rearranging Pages

#### Initial Testing (Round 6 - August/September 2024)

- Works well but lacks visual cues for easier recognition
- Recommendation: Integrate reordering into main summary view

## Page Conditions

### Evolution of Conditions Feature

#### Initial Testing (Round 2 - May/June 2024)

- "Conditions" label unclear without prompting
- "Add or change conditions" unclear; "select" condition clear
- Overall logic and clarity of conditional questions were well-understood
- Recommendation: Revise content for clarity and intuitive understanding
- Recommendation: Clarify instructions on adding conditions

#### Early Iterations (Rounds 9 & 10 - March 2025)

- [Research findings pending - these rounds focused on early iterations of the conditions feature]

#### Major Redesign (Round 11 - March 2025)

- Users confused by settings only appearing after saving a question
- Recommendation: Introduce page condition settings earlier in flow
- 'Existing vs New Condition' order led to incorrect selections
- Recommendation: Reorder 'Create new' vs 'Add existing' options
- Term "operator" caused confusion
- Preview and filter: Users skipped the filter by condition and found the preview incomplete

#### Refinement (Round 12 - March 2025)

- Participants navigated condition setup successfully
- Renaming "operator" to "Condition type" improved comprehension
- Improved understanding of new vs existing conditions
- Visibility improved by moving filter to left-hand pane
- Users appreciated the ability to check form length and condition correctness

#### Final Iteration (Round 13 - April 2025)

- Journey too repetitive—especially being asked twice to select new/existing conditions
- Recommendation: Reduce repetition in new/existing condition selection flow
- Combining multiple conditions worked well
- Checkboxes and "join" button enabled confident logic building
- Link from questions to condition setup unclear
- Users expected conditions to be tied directly to question
- Success message after condition creation helped guide users
- Info about where conditions are applied felt too subtle
- Recommendation: Present applied-condition info more prominently

### Key Improvements Over Time

1. Terminology: Changed from "operator" to "Condition type" for better clarity
2. Placement: Moved condition settings earlier in the flow
3. UI: Improved filter visibility and preview functionality
4. Workflow: Streamlined condition creation and application process
5. Feedback: Added success messages and clearer condition application indicators

### Current Recommendations

1. Reduce repetition in condition selection flow
2. Make condition–question relationships clearer in UI
3. Present applied-condition info more prominently
4. Maintain success banner with clear CTAs
5. Continue refining terminology and interaction patterns

## Page Sections

- **Round 13 (April 2025)**
  - Distinction between "section" and "page" unclear
  - Expected section headers to appear above page H1s
  - Concerns about SEO and accessibility from repeated H1/H2 usage
  - Recommendation: Clarify section vs page hierarchy with labels (e.g. H1/H2)
  - Reassess visual layout to reinforce structural expectations

## File Upload

### Evolution of File Upload Feature

#### Initial Testing (Rounds 3 & 4 - June/July 2024)

- Public view with JavaScript: Interactions and statuses clear
- Unclear why two documents were required
- Recommendation: Clarify reasons for multiple uploads
- Public view without JavaScript: Refresh page link frequently overlooked
- Recommendation: Improve visibility and clarity of refresh instructions
- Error messages: Clear but virus-related error messages need additional detail
- Recommendation: Clarify virus status and provide less alarming error presentations

#### Refinement (Round 7 - October 2024)

- File upload notification email tested well
- Clear content and interaction
- Recommendation: Maintain current clarity in notification emails

## Forms Library

### Evolution of Forms Library

#### Initial Testing (Round 1 - May 2024)

- Intuitive, similar to Whitehall Publishing
- Clear status indicators, helpful filters
- Lacks search functionality
- Users expect immediate access to personal forms
- Recommendation: Implement immediate visibility, search, and sort functionalities

#### Refinement (Rounds 3 & 4 - June/July 2024)

- Search functionality effective
- Advanced search and state filter require further investigation
- Sorting options understandable but not universally useful
- Recommendation: Evaluate usefulness further and consider streamlining options

## Form Export/Import

### Evolution of Export/Import Feature

#### Initial Testing (Round 13 - April 2025)

- Functionality tested well
- Users successfully found download option through "Tools" menu
- Maintain easy-to-locate form export/download tools

## General Improvements

### Terminology

- Standardise terms ("Published", "Live", "Change", "Edit")
- Clarify labels and process steps

### Navigation

- Implement search and sort functionalities
- Consider automatic navigation after saving changes
- Streamline navigation post-action for smoother user experience

### Content and Guidance

- Provide clearer guidance on markdown capabilities
- Improve visibility of markdown guidance
- Add contextual help for advanced options
- Implement tooltips for complex features

### User Experience

- Reduce cognitive load in complex interactions
- Improve visual hierarchy and spacing
- Maintain consistent design patterns
- Provide clear feedback for user actions

## Authentication

### Evolution of Authentication Features

#### Initial Testing (Rounds 3 & 4 - June/July 2024)

- Sign In

  - SFD sign in functionality tested
  - Clear navigation and user flow
  - Recommendation: Maintain consistent authentication patterns

- Password Reset
  - SFD reset password flow tested
  - Clear instructions and error handling
  - Recommendation: Ensure clear feedback during password reset process

## Personal Details Management

### Evolution of Personal Details Management

#### Initial Testing (Rounds 3 & 4 - June/July 2024)

- Manage personal details functionality tested
- Clear form structure and validation
- Recommendation: Maintain consistent form patterns with other Defra services

## Additional Features

### Evolution of Reference Number Format

#### Initial Testing (Round 2 - May/June 2024)

- Participants clearly understood the "minimum and maximum number of characters" and guidance
- "Short label" was unclear
- Recommendation: Revise content for clarity

### Evolution of Preview Functionality

#### Initial Testing (Round 2 - May/June 2024)

- Clear and familiar to participants
- "Get help with this form" content particularly well understood
- Preview with error messages was understood clearly

### Evolution of Guidance Only Pages

#### Initial Testing (Round 8 - November 2024)

- Confusion around how guidance pages relate to questions
- Recommendation: Revisit terminology and provide upfront context

### Evolution of Input Field Grouping

#### Initial Testing (Round 8 - November 2024)

- Common formats (names, email, phone) could be grouped as 'Written answer'
- Recommendation: Explore grouping options in future design work

### Evolution of Input Type Familiarity

#### Initial Testing (Round 8 - November 2024)

- Users understood supporting evidence, radios, checkboxes
- Autocomplete less familiar
- Recommendation: Provide clearer guidance for autocomplete and input types

### Evolution of Preview Panel

#### Initial Testing (Round 8 - November 2024)

- Helpful overall but caused confusion for radios
- Recommendation: Remove unnecessary preview items; review layout for clarity

### Evolution of Checkbox Options Entry

#### Initial Testing (Round 8 - November 2024)

- Annoying to enter options one by one
- Recommendation: Explore better bulk entry options

### Evolution of Autosave

#### Initial Testing (Round 8 - November 2024)

- Users weren't confident that progress was being saved
- Recommendation: Consider explicit save feedback (e.g. 'Save and continue')

## Check Your Answers

### Evolution of Check Your Answers Feature

#### Initial Testing (Rounds 3 & 4 - June/July 2024)

- Clear interactions and content
- Lacked essential contact and timeline information
- Users wanted to know how to chase applications and expected processing times
- Recommendation: Include contact details and expected processing times

#### Refinement (Round 5 - July 2024)

- Clearly understood but needed additional information
- Users wanted to know how to chase applications
- Recommendation: Include contact details and expected processing times

## Pet Age Input

### Evolution of Pet Age Input Feature

#### Initial Testing (Round 5 - July 2024)

- Missing options for pets younger than a year
- Users wanted input options for pet ages in months
- Recommendation: Add input options for pet ages in months

## Submission Labels

### Evolution of Submission Labels

#### Initial Testing (Round 5 - July 2024)

- Inconsistent labels ('Send', 'Submit')
- Recommendation: Follow Nielsen's usability heuristics for consistency and standards in terminology

## Visualisation and Icons

### Evolution of Visualisation Features

#### Initial Testing (Rounds 3 & 4 - June/July 2024)

- Icon representing file upload clear and effective
- Participants clearly understood icons and interactions
- Recommendation: Maintain clear and consistent iconography

## Form Designer User Journey

### Evolution of Form Designer

#### Initial Testing (Round 5 - July 2024)

- Clear overall, but confusion in some navigation paths
- Especially around creating pages and components
- Users wanted clearer navigation instructions and examples
- Recommendation: Provide clear navigation instructions, examples, and reconsider terminology ('Submit' vs 'Save')

## Internal Notes and Customisation

### Evolution of Documentation Features

#### Initial Testing (Round 1 - May 2024)

- Helpful but unclear for documenting changes
- Need for structured documentation identified
- Recommendation: Provide structured guidance for documentation

## Privacy Information

### Evolution of Privacy Features

#### Initial Testing (Round 1 - May 2024)

- Clear but excessively lengthy
- Users didn't need to read it all the time
- Recommendation: Implement accordion format for readability

## What Happens Next

### Evolution of Next Steps Feature

#### Initial Testing (Round 1 - May 2024)

- Clear content but unclear if email is only follow-up
- Recommendation: Clarify next-step actions and add preview for formatted content

## Form Preview Link

### Evolution of Preview Links

#### Initial Testing (Round 1 - May 2024)

- Confusion about necessity of preview link post-publication
- Recommendation: Clarify the function and necessity of preview links post-publication

## Tree Testing

### Evolution of Tree Testing Feature

#### Initial Testing (Round 8 - November 2024)

- Some terms didn't match users' mental models
- Recommendation: Card sort and diary study planned to improve IA
- Next steps: Conduct card sort (via Mural and Optimal Workshop) to refine guidance IA

## Form Preview and Filters

### Evolution of Preview and Filter Features

#### Initial Testing (Round 2 - May/June 2024)

- Preview with error messages was understood clearly
- "Get help with this form" content particularly well understood
- Recommendation: Maintain clear preview functionality

#### Refinement (Round 11 - March 2025)

- Users skipped the filter by condition and found the preview incomplete
- Recommendation: Improve visibility and clarity of preview and filters

#### Final Iteration (Round 12 - March 2025)

- Visibility improved by moving filter to left-hand pane
- Users appreciated ability to check form length and condition correctness
- Recommendation: Continue exposing condition filters in overview pane

## Error Message Best Practices

### Evolution of Error Message Guidelines

#### Initial Testing (Round 8 - November 2024)

- Users needed guidance on best practices for error text
- Recommendation: Provide clear guidance on error message best practices
- Users wanted flexibility in error message creation
- Recommendation: Offer flexibility while maintaining consistency

## Form Builder Flow

### Evolution of Form Builder Flow

#### Initial Testing (Round 2 - May/June 2024)

- Users preferred sketching form structure before using builder
- Recommendation: Consider providing planning tools or templates
- Mental model focused on question-first approach
- Recommendation: Align UI with question-first mental model

#### Refinement (Round 11 - March 2025)

- Tool design felt clean, well-spaced, and intuitive
- Recommendation: Maintain clean, methodical step-by-step approach
- Users wanted to see all options together during drafting
- Recommendation: Keep users on same page when adding multiple options

## Guidance and Learning Tools

### Evolution of Learning Support Features

#### Initial Testing (Round 6 - August/September 2024)

- Markdown helpful but required reminders
- Recommendation: Provide easy-access markdown help links or tooltips
- Users wanted inline help rather than external guides
- Recommendation: Improve visibility of contextual help

#### Refinement (Round 7 - October 2024)

- Unclear if markdown usage had limitations
- Recommendation: Provide detailed guidance on markdown capabilities
- Users needed clearer examples and context
- Recommendation: Add more contextual examples and guidance

## Form Creation Flow

### Evolution of Form Creation Flow

#### Initial Testing (Round 2 - May/June 2024)

- Users preferred a linear, intuitive flow from form name to question creation
- Mental model focused on question-first approach
- Recommendation: Consider automatically creating pages as users add questions

#### Refinement (Round 11 - March 2025)

- Tool design felt clean, well-spaced, and intuitive
- Recommendation: Maintain clean, methodical step-by-step approach
- Users wanted to see all options together during drafting
- Recommendation: Keep users on same page when adding multiple options

#### Final Iteration (Round 12 - March 2025)

- Moving question text before options improved logic and usability
- No usability issues observed; setup was clear
- Recommendation: Retain improved order of question-before-options

## Visual Design and Navigation

### Evolution of Visual Design

#### Initial Testing (Round 1 - May 2024)

- Intuitive, similar to Whitehall Publishing
- Clear status indicators, helpful filters
- Recommendation: Maintain consistent design patterns

#### Refinement (Round 11 - March 2025)

- Tool design felt clean, well-spaced, and intuitive
- Recommendation: Maintain clean, methodical step-by-step approach

#### Final Iteration (Round 13 - April 2025)

- Overall design intuitive and easy to use
- Success messages and clear CTAs particularly helpful
- Recommendation: Maintain success banner with clear CTAs

## Form Preview and Filters

### Evolution of Preview and Filter Features

#### Initial Testing (Round 2 - May/June 2024)

- Preview with error messages was understood clearly
- "Get help with this form" content particularly well understood
- Recommendation: Maintain clear preview functionality

#### Refinement (Round 11 - March 2025)

- Users skipped the filter by condition and found the preview incomplete
- Recommendation: Improve visibility and clarity of preview and filters

#### Final Iteration (Round 12 - March 2025)

- Visibility improved by moving filter to left-hand pane
- Users appreciated ability to check form length and condition correctness
- Recommendation: Continue exposing condition filters in overview pane

## Upload/Download Features

### Evolution of File Management Features

#### Initial Testing (Rounds 3 & 4 - June/July 2024)

- Public view with JavaScript: Interactions and statuses clear
- Unclear why two documents were required
- Recommendation: Clarify reasons for multiple uploads

#### Refinement (Round 7 - October 2024)

- File upload notification email tested well
- Clear content and interaction
- Recommendation: Maintain current clarity in notification emails

#### Final Iteration (Round 13 - April 2025)

- Functionality tested well
- Users successfully found download option through "Tools" menu
- Recommendation: Maintain easy-to-locate form export/download tools
